...
- [x] **定选题** —— v4 收口到「小哒 Coda：扫盘 Review / 行业画像（真做）+ 跨项目复用（医疗最小切片）」（见 [`选题定稿.html`](./选题定稿.html)）
- [x] **Agent 接口契约** —— `/profile` + `/review` 两个 API，给 Agent 同学照搭（见 [`Agent接口契约.html`](./Agent接口契约.html)）
- [ ] **改 PRD / 路演 PPT 到 v4** —— 当前 [`PRD.html`](./PRD.html) / [`路演PPT.html`](./路演PPT.html) 仍是 v3.0「插件裂变」主线，需随 v4 改主线（**待办，未改**）
- [ ] **扫盘引擎** —— 遍历本地目录 + 中英文医疗关键词库 + 反推画像，输出 `{行业, 证据词, 文件清单}`
- [ ] **Agent 层** —— 实现 `/profile` 反推画像 + `/review` 行业翻译/打分/改法（只读不改）
- [ ] **前端 / 桌宠** —— 桌宠入口 + 画像卡可编辑 + 报告页 + 复用提醒弹窗
- [ ] **医疗行业能力库** —— 从项目 A 真抽 1~2 个轮子（患者 ID 校验 / 就诊状态机）入库，供项目 B 复用
- [ ] **预备两个医疗项目素材** —— 项目 A（已扫、轮子已入库）+ 项目 B（当场扫）
- [ ] **Demo 公网链接 + 完整录屏**（扫盘→画像→翻译报告全流程录一条兜底）
- [ ] 路演演练 ≥2 次，卡时长
