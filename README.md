...
> **小哒 Coda** 是一个常驻桌面、懂你行业的 Coding Agent 助手——
> 它盯着你的 Coding Agent **实时反馈进度**（看得见），带**业务维度**帮你 Review（懂业务），还能把行业经验**打包成插件一键分享**给同行（能裂变）。
...
- [x] **定选题** —— 收口到「小哒 Coda：桌面助手 + 三能力」（见 [`选题定稿.html`](./选题定稿.html)）
- [x] **PRD + 路演 PPT** —— 产品中心叙事、命名统一小哒 Coda（[`PRD.html`](./PRD.html) / [`路演PPT.html`](./路演PPT.html)）
- [ ] **搭真插件仓库** —— 让第 7 页 `/plugin install` 现场真能跑（Demo 最高潮来源）
- [ ] **桌面助手原型录屏** —— 形象常驻 + 状态反馈 + 权限气泡（补 PPT 第 7 页）
- [ ] **装前 vs 装后对比截图** —— 测对比问题组，找差异最显著的一帧
- [ ] **Demo 公网链接 + 完整录屏**
- [ ] 路演演练 ≥2 次，卡 4 分钟
...
| 文件 | 用途 |
|---|---|
| [`路演PPT.html`](./路演PPT.html) | 路演演讲稿 · 8 页 · 深蓝暖金风（封面海报 + 痛点 + 产品亮相 + 三能力 + 现场 Demo + 收尾） |
| [`PRD.html`](./PRD.html) | 产品需求文档 v3.0 · 产品中心版（定位/问题/画像/产品形态=常驻桌面助手/三能力/技术/功能/MVP/Demo/指标/风险/演进） |
